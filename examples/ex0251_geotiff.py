...
"""
Chiginagak volcano
Showing 3 layers of hexagons: geology, land-usage, hill-shading
...
def plot_hex(overlays, name='hex', bbox=None, draw_bbox=True, nodata=0, shade_strength=0.65):
    ...
    dpi = 500
    adj = (72 / dpi)
    fig = plt.figure(figsize=(17, 10), dpi=dpi, frameon=False)
    ...
    ax.set_aspect('equal', adjustable='box')
    ax.set_axis_off()
    fig.savefig(f"output/ex0251_whx_{name}.png", dpi=dpi)
    print(f"fig saved at output/ex0251_whx_{name}.png")
...
if __name__ == '__main__':
    ...
    rock_lut = create_label_lut('src/chiginagak_geology.json')
    ...
    hs_ds = gdal.Open("src/hs_md.tif")  # Grab the hillshade for this area.
    rocks = gdal.Open('src/chiginagak_geology.tif')
    ...
    focus = "APNPr"
    size, size_str = 4_500.0, '4_5k'        # width/height of map, in metres.
    rcc = Points(np.atleast_2d([57.13330344238441, -156.98699788901348]), g_gcd)
    ...
    for layer in [10]:  # range(8, 11):
        ...
        # geology at L-1 (coarser)
        rock_delta = -1
        rock_pts = tri_grid_clipped(level=(layer + rock_delta) + 3, mode=int(mo[0]), bbox=bbox)
        rock_bt = Points(rock_pts, b_oct, components=bhx.components[0])
        rock_smp = rg.project(rock_bt, [b_oct, c_oct, c_ell, g_wkt])
        # rock_nt = rg.project(rock_bt, [b_oct, n_oct])
        rock_bt.samples, shade_px_id = sample_gdal(rocks, rock_smp.coords)  # see below
        rock_bt.px_id = shade_px_id
        rock_hx_pts, rock_v = hex_poly_layer(rock_bt, layers=layer + rock_delta, reducer=mode_u8)
        rock_v = rock_v.astype(np.uint8)

        # hillshade tone at L+1 (denser)
        shade_delta = 1
        shade_pts = tri_grid_clipped(level=(layer + shade_delta) + 3, mode=int(mo[0]), bbox=bbox)
        shade_bt = Points(shade_pts, b_oct, components=bhx.components[0])
        shade_smp = rg.project(shade_bt, [b_oct, c_oct, c_ell, g_wkt])
        ...
        rok_n = rg.project(rock_hx_pts, [b_oct, n_oct])
        ...
        overlays = [[use_n, lu_vals, nlcd_lut], [shd_n, sh_vals, None], [rok_n, rock_v, rock_lut]]
        plot_hex(overlays, f'{size_str}L{layer:02d}{focus}', bbox=bbox_n)
