...
Last Tested
26 December 2025 0.1.0a4 (working well)
"""
import time
...
def plot_hex(pts, name='hex', ctrs=None, values=None, lut=None, nodata=0):
    """Plot hex polygons.
    Args:
    ...
    if ctrs is not None:
        ax.scatter(ctrs.coords[:, 0], ctrs.coords[:, 1], ec='none', marker='.', s=30)
    ax.set_aspect('equal', adjustable='box')
    ax.set_axis_off()
...
def scanline_h9_sheet(poly_n, level):
    # sn is your grid spacing factor
    ...
    result = Points(uv, domain=poly_n.domain)
...
if __name__ == '__main__':
    ...
    for layer in range(4, 10):
        with time_block(f"{layer} scanline_h9_sheet"):
            ...
            hx_pts, vals = hex_poly_layer(b_pts, layers=layer)
            h_pts = rg.project(hx_pts, [b_oct, n_oct])
            plot_hex(h_pts, f'{layer}')
