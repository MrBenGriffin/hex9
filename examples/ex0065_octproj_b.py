...
Last Tested
16 Jun 2026 0.1.3a0 (passed) 17.8s
"""
...
def octant_plot(reg: Registrar, depth: int, octant_id: int = 0) -> None:
    b_pts, _ = get_octant_data(reg, depth)
    ...
    dx = n_pts[:, 0] -  c_pts.coords[:, 0]
    dy = n_pts[:, 1] -  c_pts.coords[:, 1]
    dz = n_pts[:, 2] -  c_pts.coords[:, 2]
    dist3 = [dx, dy, dz]
    ...
    for axis in range(3):
        ...
        ax.scatter(b_pts.coords[:, 0], b_pts.coords[:, 1], c=dist, cmap=cmap, norm=norm, s=0.5, alpha=1.0)
        ax.autoscale_view()
        # Colorbar
