...
class Util:
    """
     ...
     • Common, unadorned GCD calculations (spherical).
      xyz_ll - convert Euclidean coordinates to geographic coordinates.
      ll_xyz - convert geographic coordinates to Euclidean coordinates.
     • JSON handlers
    ...
    def o_valid(self, pts: NDArray) -> bool:  # Constraint: |u|+|v|+|w|=1 (surface of the unit octahedron)
        """
        Test that |u|+|v|+|w|=1 (surface of the unit octahedron)
    ...
    def s_valid(self, pts: NDArray) -> bool:  # Constraint: √(u^2+v^2+w^2)=1 (surface of the unit sphere)
        """
    ...
    def oct_rnd(self, n):
        o0 = self.oct0_rnd(n)  # 2d triangles.
        signs = [
