...
class H9:
    ...
    def __init__(self, owner: Drawing, identity: str = 'h9', size: float = 81., stroke: float = 0.5, op=0.9):
        self.owner = owner
        # self.rgb = rgb
        self.opacity = op
        self.id_ref = f'#{identity}'
    ...
    def place_district(self, where: list, color: str, district: int):
        districts = [
    ...
    def place(self, where: list, colors: list):
        for i in range(18):
            self.place_district(where, colors[i], i)
...
if __name__ == '__main__':
    ...
    canvas = Drawing('test6', (600, 600), False)
    h1 = H2(canvas, 'h1', 9., 0, op=0.90)
    # for j in range(-6, 6):
