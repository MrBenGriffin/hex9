...
def _worker_project_gcd_to_bary(chunk_ll: np.ndarray):
...
def _project_gcd_to_boct_parallel(ll_array: np.ndarray, workers=0, chunk=8_000):
    """Paralleled work"""
    reg = Registrar()
    if workers <= 0:
        import os
    ...
    with ProcessPoolExecutor(max_workers=workers) as ex:
        ...
        try:
            for xy, cmp in ex.map(_worker_project_gcd_to_bary, chunks):
...
class GCDBary(Projection):
    ...
    def __init__(self, registrar, *, workers: int | None = None, chunk: int = 8_000, threshold: int = 8_000, accuracy: float = 1e-10):
        ...
        self.ake.set_accuracy(self.accuracy)
    ...
    def set_accuracy(self, meters: float):
        ...
        ake = self.reg.projection('oct_ell')
        deg = ake.set_accuracy(meters)
        return deg
    ...
    def forward(self, arr: Points) -> Points:
        ...
        if n < self.threshold:
            ...
            bc = self.reg.project(arr, ['g_gcd', 'c_ell', 'c_oct', 'b_oct'])
            return bc
        xy, cmp = _project_gcd_to_boct_parallel(coords, workers=self.workers or 0, chunk=self.chunk)
