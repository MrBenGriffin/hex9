...
class OctantBraw(Projection):
    """
    This is the 3D<->2D-Barycentric projection (unwarped)
    ('b_raw', 'c_oct')
    """

    def __init__(self, registrar, base, o_name, b_name):
        # print(f'> OctantBraw: {base}_brw; {o_name} -> {b_name}')
        super().__init__(registrar, f'{base}_brw', o_name, b_name)
        self.matrix = None
