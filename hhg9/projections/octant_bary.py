...
class OctantBary(Projection):
    """
    This was the 3D<->2D-Barycentric projection.
    But now is the b_raw / b_oct projection - and b_raw manages 2D/3D.
    """

    def __init__(self, registrar, base, o_name, b_name):
        # print(f'> OctantBary: {base}_ob; {o_name} -> {b_name}')
    ...
    def forward(self, arr) -> Points:
        ...
        pts = arr.coords if isinstance(arr, Points) else arr
        if self.warp is not None:
            pts = self.warp.undo(pts, self.mode)
        if isinstance(arr, Points):
            return Points(pts, domain=self.fwd_cs, samples=arr.samples, oid=arr.oid)
        else:
            return pts
    ...
    def backward(self, arr: Points) -> Points:
        ...
        if isinstance(arr, Points):
            return Points(xy, domain=self.rev_cs, samples=arr.samples, oid=arr.oid)
        else:
            return xy
