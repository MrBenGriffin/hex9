...
def find_coords(target_rll, initial_mode, target_octants, h9c: H9CellLike,
    ...
    target_rll: (layer,2) lat/lon (or whatever the projector uses)
    initial_mode: (layer,) int {0,1}
    target_octants: (layer,3) face-signs for projection
    ...
    """
    num_pts = target_rll.shape[0]  # layer
    off = h9c.off_xy
    ...
    # Beam state
    best_paths = root_uris[:, None, None]  # (layer,1,1)
    best_xy = np.zeros((num_pts, 1, 2), dtype=np.float64)  # (layer,1,2)

    for i in range(depth):
        k = best_paths.shape[1]  # current beam width
        last = best_paths[:, :, -1]  # (layer,k)
        par_mode = mode_lut[last]  # (layer,k)

        # Select children per parent mode
        children = np.where(par_mode[..., None] == 1, up_children, down_children)  # (layer,k,9)

        # Expand candidate URIs and coords
        cand_uris = children.reshape(num_pts, k * 9)  # (layer,k*9)
        scale = (1.0 / 3.0) ** i
        par_xy = np.repeat(best_xy, 9, axis=1)  # (layer,k*9,2)
        offs = off[cand_uris]  # (layer,k*9,2)
        cand_xy = par_xy + offs * scale  # (layer,k*9,2)
        ...
        d = distance_func(proj, target_rll[:, None, :])  # (layer,k*9)

        # Top-k via arg-partition, then stable tie-break by URI
        idx_k = np.argpartition(d, beam_width - 1, axis=1)[:, :beam_width]  # (layer,k)
        # Gather those distances/uris for stable sort
        d_k = np.take_along_axis(d, idx_k, axis=1)  # (layer,k)
        u_k = np.take_along_axis(cand_uris, idx_k, axis=1)  # (layer,k)
        # Stable rank on (distance, uri)
        order = np.lexsort((u_k, d_k))  # (layer,k)
        top_idx = np.take_along_axis(idx_k, order, axis=1)  # (layer,k)

        # Keep winners
        win_uris = np.take_along_axis(cand_uris, top_idx, axis=1)  # (layer,k)
        win_xy = np.take_along_axis(cand_xy, top_idx[..., None], axis=1)  # (layer,k,2)

        # Extend paths: pick corresponding parent slices
        parent_sel = top_idx // 9  # (layer,k)
        parent_paths = np.take_along_axis(best_paths, parent_sel[..., None], axis=1)  # (layer,k,depth_so_far)
        best_paths = np.concatenate([parent_paths, win_uris[..., None]], axis=2)  # (layer,k,depth_so_far+1)
        best_xy = win_xy
