...
def find_coords(target_rll, initial_mode, target_octants, h9c: H9CellLike,
    ...
    # Beam state
    best_paths = root_uris[:, None, None]  # (hex_layer,1,1)
    best_xy = np.zeros((num_pts, 1, 2), dtype=np.float64)  # (hex_layer,1,2)
    ...
    for i in range(depth):
        ...
        cand_uris = children.reshape(num_pts, k * 9)  # (hex_layer,k*9)
        scale = (1.0 / 3.0) ** i
        par_xy = np.repeat(best_xy, 9, axis=1)  # (hex_layer,k*9,2)
        offs = off[cand_uris]  # (hex_layer,k*9,2)
        cand_xy = par_xy + offs * scale  # (hex_layer,k*9,2)
        ...
        order = np.lexsort((u_k, d_k))  # (hex_layer,k)
        top_idx = np.take_along_axis(idx_k, order, axis=1)  # (hex_layer,k)

        # Keep winners
        win_uris = np.take_along_axis(cand_uris, top_idx, axis=1)  # (hex_layer,k)
        win_xy = np.take_along_axis(cand_xy, top_idx[..., None], axis=1)  # (hex_layer,k,2)

        # Extend paths: pick corresponding parent slices
        parent_sel = top_idx // 9  # (hex_layer,k)
