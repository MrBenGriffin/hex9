...
STEP_COOL     = 0.85      # multiply lr by this when MAE improvement stalls
...
if __name__ == '__main__':
    ...
    x_prime[lat_mask]   = l5_src[lat_mask]
    ...
    for k in range(ITERATIONS):
        ...
        best = False
        if (mae < best_mae and curr_min >= best_min and curr_max <= best_max) or (mae == best_mae and ((curr_min > best_min) or (curr_max < best_max))):
        ...
        if (k % 23 == 0) or best:
            out_path = Path(f"l05_{ellipsoid}_k{k:05d}.npz")
            np.savez(
                out_path,
                layer=LAYER,
                source_pts=l5_src,
