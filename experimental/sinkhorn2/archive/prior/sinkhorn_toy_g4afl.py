...
def afl(reg, b_oct, octant_id, mode, layer, marker, iterations):
    ...
    for i in range(FEEDBACK_ITERATIONS):
        ...
        # Apply Correction
        # If Ratio > 1 (Too big), we Increase Weight to pull it in.
        # current_strength = max(0.1, FEEDBACK_STRENGTH * (0.90 ** i))
        if i < 200:
            # Phase 1: Aggressive correction to kill the bands
            current_strength = 1.0
        # elif i < 31:
        #     # Phase 2: Moderate correction for corner/pinned stress
        #     current_strength = 0.75
        # elif i < 60:
        #     # Phase 3: Fine tuning (Floor raised to 0.25 to prevent stalling)
        #     current_strength = 0.50
        # else:
        #     current_strength = 0.25
        current_b_w *= np.power(v_corr, current_strength)
        current_b_w /= current_b_w.sum()

        print(f"   iter:{i:02d}; current_strength:{current_strength:.6f}")

        # Save Plot
        marker = f"fb_iter{i:02d}_{int(current_strength * 1e6):07d}"
...
if __name__ == '__main__':
    ...
    #
    # # A. Create Ghosts
    # # We reflect both Source (a_p) and Target (t_p)
    # # We reflect both Source Weights (a_w) and Target Weights (b_w)
    # ghost_a, full_a_w, _ = create_ghost_padding(a_p, a_w)
    #
    # # For the target, we use the specific t_p and b_w we calculated
    # ghost_t, full_b_w, mask = create_ghost_padding(t_p, b_w)
    #
    # # B. Run Sinkhorn on the Super-Set
    # loss_matrix = ot.dist(ghost_a, ghost_t, metric="sqeuclidean")
    # loss_matrix /= loss_matrix.max()
    # best = 0.00020849
    # for REG in np.linspace(best - 1e-6, best + 1e-6, 11):
    #     reg_m = f'{int(REG * 1e8):06d}'
    #     mark = f'{marker}_{reg_m}'
    #
    #     gamma_log = ot.sinkhorn(
    ...
    #
    #     row_sum = gamma_log.sum(axis=1, keepdims=True)
    #     x_super = (gamma_log @ ghost_t) / np.maximum(row_sum, 1e-30)
    #     x_prime = x_super[mask]
    #
    #     # 4. Analytic Snap (Final Polish)
    #     x_prime = snap_boundary_analytic(x_prime, oc_edg, oc_vtx, net_mode)
    #     dpts = Points(x_prime, b_oct, cmp)
    #     gpts = rg.project(dpts, [b_oct, g_gcd])
    ...
    #     print(f'REG: {REG:.9f}')
    #
    #     plot_density(b_oct, rx, x_prime, t_grid, octant_id, layer, mark, oc_vtx=oc_vtx)
    #     if do_vectors:
