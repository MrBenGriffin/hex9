...
if __name__ == '__main__':
    ...
    move_mask[lat_mask] = False      # lateral edges pinned to zero displacement
    x_prime[lat_mask]   = l4_src[lat_mask]
    ...
    for k in range(start_iter, start_iter + ITERATIONS):
        ...
        is_best = (mae < best_mae and curr_min >= best_min and curr_max <= best_max) or (mae == best_mae and ((curr_min > best_min) or (curr_max < best_max)))
        ...
        if (k % SAVE_EVERY == 0) or is_best:
            ...
            out_path = Path(f'l4_tgrid_{ellipsoid}_{tag}.npz')
            np.savez(
                out_path,
