...
if __name__ == '__main__':
    ...
    #
    # Cooling: the previous trigger (Δmae < 1e-4) fired on natural noise once the
    # 0.75/0.3 blend amplification was removed, freezing strength at the floor
    # before the worst cells were resolved. Tighten the plateau threshold and
    # lift the floor so late iters still carry useful correction force.
    STRENGTH_FLOOR = 0.25
    PLATEAU_THR    = 1e-5
    for i in range(start_iter, FEEDBACK_ITERATIONS):
        # A. Cooling Schedule (MAE-gated)
        if mae_prev - mae < PLATEAU_THR:
            strength = max(strength * 0.85, STRENGTH_FLOOR)
        mae_prev = mae
        print(f"\n[Iter {i + 1}] Solving (Strength={strength:.2f})...")
        sh_verbosity = i == start_iter
        ...
        with warnings.catch_warnings(record=True):
            ...
            gamma = ot.sinkhorn(gaw_use, gbw_use, M_cache, reg=REG,
                                # , method='sinkhorn_log',
                                numItermax=50000, stopThr=1e-7, verbose=sh_verbosity, warn=True)
