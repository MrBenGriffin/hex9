...
L5_CHECKPOINT = Path("l5_grad_32_s5e-06_00100.npz")   # Set to resume an existing L5 run; None = fresh start
OCTANT_ID     = 0
LAYER         = 5
...
if __name__ == '__main__':
    ...
    for k in range(ITERATIONS):
        ...
        print(f"[Iter {k+1:4d}]  MAE: {mae:.8f} | lr={step:.1e} | "
              f"All Min/Max: {ratios.min():.6f}/{ratios.max():.6f} | "
              f"Seam Min/Max: {seam_r.min():.8f}/{seam_r.max():.8f}")
        ...
        if k % 100 == 0:
            out_path = Path(f"l5_grad_{SEAM_RINGS}_s{STEP_SIZE}_{k:05d}.npz")
            np.savez(
                out_path,
                layer=LAYER,
                source_pts=l5_src,
