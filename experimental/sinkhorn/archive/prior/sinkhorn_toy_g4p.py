...
def plot_grid(grid_pts,
    ...
    cb.ax.tick_params(labelsize=16)  # or 14, etc.
...
def plot_density(b_oct, c_den, vts, trx, octant_id, layer, marker, oc_vtx=None):
    ...
    plot_grid(b_grid, fc_n=acol_norm, title=file_title, text=f'mae:{c_mae}; p99:{c_p99}')
...
if __name__ == '__main__':
    ...
    loss_matrix /= loss_matrix.max()
    best = 0.00020869
    for REG in np.linspace(best - 1e-5, best + 1e-5, 11):
