...
REG = 0.00008      # Sinkhorn regularisation. L3 test value; tighten (e.g. 0.00008) for L4
FEEDBACK_ITERATIONS = 100
START_STRENGTH = 0.78  # Lower start strength to prevent initial ringing
RESUME_FROM = None  # Set to e.g. "output/q_WGS84_l4_iter23.npz" to resume after a crash
WARM_WEIGHTS_FROM = None  # Path to a Sinkhorn checkpoint (any layer) whose
                          # target_weights are CT-interpolated onto this layer's
                          # vertices as the initial b-weight distribution.
                          # Typical workflow: solve L3 first (fast / small RAM),
                          # then warm-start L4 weights from that L3 result so the
                          # solver begins near the correct mass redistribution
                          # rather than the raw exp(v_ell) prior.
...
if __name__ == '__main__':
    import argparse
    ap = argparse.ArgumentParser(description='Stage 1 — Sinkhorn OT warp (L4).')
    ap.add_argument('--ellipsoid', default=config.INDEX_ELLIPSOID,
                    choices=list(config.ELLIPSOIDS))
    ap.add_argument('--resume', default=RESUME_FROM,
                    help='checkpoint .npz to resume from')
    ap.add_argument('--warm-weights', default=WARM_WEIGHTS_FROM,
                    help='Sinkhorn checkpoint (any layer) to CT-interp target_weights from')
    ap.add_argument('--bump-strength', type=float, default=None,
                    help='After --resume, override the restored strength (e.g. 0.66) '
                         'to undo premature cooling.')
    ap.add_argument('--layer', type=int, default=LAYER,
                    help='Hex layer to solve on (default %(default)s). Use a small '
                         'value (e.g. 2) for fast sanity tests.')
    ap.add_argument('--max-iter', type=int, default=FEEDBACK_ITERATIONS,
                    help='Cap on outer feedback iterations.')
    # ap.add_argument('--method', default='sinkhorn')
    ap.add_argument('--no-warmstart', action='store_true',
                    help='Disable Sinkhorn dual-potential warm-starting (A/B baseline).')
    ap.add_argument('--verbose-all', action='store_true',
                    help='Force POT verbose output on every outer iter (timing/debug).')
    args = ap.parse_args()
    resume_from = args.resume
    warm_weights_from = args.warm_weights
    LAYER = args.layer
    FEEDBACK_ITERATIONS = args.max_iter
    ...
    # No lateral-edge identity pin: edges are kept straight by the ghost halo +
    # snap_boundary_analytic projection; vertices on the edge are free to slide
    # along the line. Forcing them to a_p was previously starving the seam-
    # adjacent rows of mass and locking in the worst min/max outliers.
    # Only the three octant corners (oc_vtx) are immovable.
    ...
    #
    # No percentile clipping: the worst-distorted vertices (apex / seam neighbourhood)
    # need the full dynamic range of the target weight signal to attract enough mass.
    # Clipping at p2/p98 was previously truncating exactly those cells, leaving the
    # min/max ratios stuck around 0.67 / 1.30 even after many iterations.
    num = len(pts)
    a_w = np.ones(num, dtype=np.float64) / num
    b_w_raw = np.exp(v_ell - np.median(v_ell))
    current_b_w = b_w_raw / b_w_raw.sum()

    # Optional warm start of the b-weight distribution from a lower-layer
    # Sinkhorn solution. Interpolation is done in log-space so the multi-order
    # dynamic range of the converged weights survives the resampling.
    # Skipped if --resume is also given (resume restores the full weight state
    # directly from the L4 checkpoint).
    if warm_weights_from is not None and resume_from is None:
        from scipy.interpolate import CloughTocher2DInterpolator, NearestNDInterpolator
        ww_path = Path(warm_weights_from)
        print(f"Warm-starting b-weights from {ww_path} ...")
        ww = np.load(ww_path, allow_pickle=True)
        if 'target_weights' not in ww.files:
            raise ValueError(f"{ww_path} has no 'target_weights' field")
        if 'source_pts' in ww.files:
            ww_src = ww['source_pts']
        elif 'layer' in ww.files:
            ww_layer = int(ww['layer'])
            ww_src = np.load(Path(f"grid_l{ww_layer}.npz"),
                             allow_pickle=True)['xy_vert']
        else:
            raise ValueError(f"{ww_path} has no source_pts or layer to resolve vertices")
        ww_w = np.asarray(ww['target_weights'], dtype=np.float64)
        log_w = np.log(np.maximum(ww_w, 1e-30))
        ct = CloughTocher2DInterpolator(ww_src, log_w)
        log_w_new = ct(a_p)
        nan_mask = np.isnan(log_w_new)
        if nan_mask.any():
            print(f"  NearestND fallback for {int(nan_mask.sum())} vertices outside hull")
            nn = NearestNDInterpolator(ww_src, log_w)
            log_w_new[nan_mask] = nn(a_p[nan_mask])
        # Subtract max for numerical stability before exp; renormalise to a pmf.
        new_w = np.exp(log_w_new - log_w_new.max())
        current_b_w = new_w / new_w.sum()
        print(f"  Warm-started {len(current_b_w)} L{LAYER} weights "
              f"from {len(ww_src)}-pt source.")
    ...
    start_iter = 0
    # Sinkhorn dual potentials (linear domain). Persisted across outer iters so
    # the inner Sinkhorn can warm-start from the previous solution rather than
    # restarting from u=v=1 every time. b changes only slightly between outer
    # iters, so the prior (u,v) is already a near-fixed-point — typically cuts
    # inner iter count 2-5x. Sinkhorn is strictly convex: warm-start changes the
    # path to the fixed point, never the fixed point itself.
    u_prev = None
    v_prev = None
    ...
    if resume_from is not None:
        ...
        mae         = float(ckpt['mae'])
        u_prev      = ckpt['u_prev'] if 'u_prev' in ckpt.files else None
        v_prev      = ckpt['v_prev'] if 'v_prev' in ckpt.files else None
        if u_prev is not None and u_prev.dtype == object:
            u_prev = None  # legacy checkpoints stored None as 0-d object array
            v_prev = None
        start_iter  = int(ckpt['iteration']) + 1
        print(f"  Resuming at iter {start_iter}, strength={strength:.3f}, MAE={mae:.8f}")
        if args.bump_strength is not None:
            print(f"  --bump-strength: {strength:.3f} → {args.bump_strength:.3f}")
            strength = float(args.bump_strength)
    ...
    #
    # Cooling: gate is now *relative* (ΔMAE / MAE). An absolute 1e-5 floor fired
    # spuriously once MAE itself dropped to ~5e-3 — late iters were still making
    # ~0.2%/iter progress but each Δ slipped just under 1e-5, so cooling
    # compounded every step and starved the worst-distorted cells of correction
    # force. Relative threshold stays meaningful regardless of MAE scale.
    STRENGTH_FLOOR  = 0.25
    PLATEAU_THR_REL = 1e-3     # cool when relative improvement < 0.1%
    for i in range(start_iter, FEEDBACK_ITERATIONS):
        # A. Cooling Schedule (relative-MAE-gated)
        rel_improvement = (mae_prev - mae) / max(mae_prev, 1e-12)
        if rel_improvement < PLATEAU_THR_REL:
            strength = max(strength * 0.85, STRENGTH_FLOOR)
        mae_prev = mae
        t_iter_start = time.perf_counter()
        print(f"\n[Iter {i + 1}] Solving (Strength={strength:.2f})...")
        sh_verbosity = args.verbose_all # or (i == start_iter)
        ot_method = 'sinkhorn' # if i == start_iter else args.method
        ...
        # C. Sinkhorn
        # warmstart expects log-domain potentials; POT internally exponentiates.
        # On the first outer iter (u_prev is None) we let POT use its default
        # u=v=1/dim initialisation.
        sk_kwargs = dict(reg=REG, numItermax=50000, stopThr=1e-7,
                         method=ot_method,
                         verbose=sh_verbosity, warn=True, log=True)
        if u_prev is not None and not args.no_warmstart:
            # Compute the log in float64 — a 1e-300 floor underflows to 0 in
            # float32 (smallest float32 ≈ 1.18e-38), so the floor only protects
            # zeros when promoted.
            sk_kwargs['warmstart'] = (
                np.log(np.maximum(u_prev.astype(np.float64), 1e-300)).astype(np.float32),
                np.log(np.maximum(v_prev.astype(np.float64), 1e-300)).astype(np.float32),
            )
        t_sk_start = time.perf_counter()
        with warnings.catch_warnings(record=True):
            warnings.simplefilter("always", UserWarning)
            gamma, sk_log = ot.sinkhorn(gaw_use, gbw_use, M_cache, **sk_kwargs)
        t_sk = time.perf_counter() - t_sk_start
        u_prev = np.asarray(sk_log['u'], dtype=np.float32).copy()
        v_prev = np.asarray(sk_log['v'], dtype=np.float32).copy()
        ws_tag = 'cold' if (args.no_warmstart or i == start_iter) else 'warm'
        print(f"   Sinkhorn ({ws_tag}): {t_sk:.2f}s")

        # D. Reconstruct: barycentre of transport plan, strip ghost rows, snap boundary
        print(f"   snapping boundary analytic...")
        ...
        print(f"   Calculating wgs84_area...")
        ...
        if i > 0:
            # Coefficients must sum to 1.0 — earlier 0.75 + 0.3 = 1.05 amplified
            # corrections by 5% per iter, biasing the schedule toward oscillation
            # and forcing the strength floor to fire prematurely.
        ...
        np.savez(
            f"output/q_{ellipsoid}_l{LAYER}_iter{i}.npz",
            mae=mae,
            source_pts=a_p,
            target_pts=x_prime,
