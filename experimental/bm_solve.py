...
class DiGraphSolver:
    ...
    def solve(self) -> bool:
        ...
        cp_solver.parameters.max_time_in_seconds = 1600
...
class NetworkDigraph:
    ...
    """
    def __init__(self, nodes, edges, start, stop):
        self.steps = 1
        self.start = start
        self.stop = stop
        self.nodes = dict()
        for node in nodes:
            self.nodes[node] = []
            # self.nodes[node] = [node] if node != start else []
            for (a, b, w) in edges:
                if node == a:
...
def solve_az_paths_of_a_digraph(nodes, edges, start, stop):
    problem = NetworkDigraph(nodes, edges, start, stop)
    # problem.print_nodes()
    # print()
    # min_steps = solve_with_steps(problem, 'min', True)
    max_steps = solve_with_steps(problem, 'max', True)
...
if __name__ == '__main__':
    ...
    solve_az_paths_of_a_digraph(nodes, edges, '0e000s', '0e001n')
