...
def plot_density(b_oct, c_den, vts, trx, octant_id, layer, marker):
    ...
    min = np.min(c_val)
    max = np.max(c_val)
    print(f"mae: {mae}; std: {std}; min: {min}; max: {max}")
...
if __name__ == '__main__':
    ...
    CENTER_BOOST = 3.0  # fights the "Vacuum Effect" (the sheet shrinking inward).
    CORNER_BOOST = 1.3  # Low: tips tear away or stretch violently. High: Corners get crushed
    TAPER_PC = 0.25  # Percentage of where to taper.
    ...
    h_start, h_stop, h_step = 1.0, 1.5, 15
    hyper = np.linspace(h_start, h_stop, h_step)
    print(f"c. hyper: ({h_start} to {h_stop}; {h_step} steps) across CENTER_BOOST")
    for CENTER_BOOST in hyper:
        cts = f'_ct{int(100 * CENTER_BOOST):03d}'
        cns = f'_cn{int(100 * CORNER_BOOST):03d}'
        tps = f'_tp{int(100 * TAPER_PC):03d}'
        ...
        if d_max > 0:
            taper = np.clip(d_edge / (TAPER_PC * d_max), 0.0, 1.0)
            b_w_raw *= (1.0 + (CENTER_BOOST - 1.0) * taper)
        ...
        plot_density(b_oct, rx, x_prime, t_grid, octant_id, layer, marker)
