...
def run(layer, best_path, teacher_model):
    lim = H9K.limits
    rg = Registrar()
    ...
    env_order = float(teacher_model.get("env_order", 1.0))
    eps_uvw = float(teacher_model.get("eps_uvw", 0.0))
    ridge = float(teacher_model.get("ridge", 0.0))
    edge_guard_m_end = int(teacher_model.get("edge_guard_m_end", 32))
    edge_guard_power = float(teacher_model.get("edge_guard_power", 2.0))
    edge_beta = float(teacher_model.get("edge_beta", 0.0))

    # Some historical models store these as NaN when unset; fall back to defaults.
    if not np.isfinite(env_alpha):
        env_alpha = 0.60
    if not np.isfinite(env_order):
        env_order = 1.0
    if not np.isfinite(eps_uvw):
        eps_uvw = 0.0
    if not np.isfinite(ridge):
        ridge = 0.0
    if not np.isfinite(edge_guard_power):
        edge_guard_power = 2.0
    if not np.isfinite(edge_beta):
        edge_beta = 0.0
    ...
    edge_maps = build_edge_maps(
        ...
        mode=mode,
        oc_edg=oc_edg,
        guard_m_end=int(edge_guard_m_end),
        guard_power=float(edge_guard_power),
    ...
    save_kwargs = dict(
        ...
        eps_uvw=eps_uvw,
        enforce_lr_symmetry=True,
        edge_guard_m_end=int(edge_guard_m_end),
        edge_guard_power=float(edge_guard_power),
        edge_beta=float(edge_beta)
    )
    save_kwargs.update(dict(
        ...
        # Pts. mapping
        src_pts=a_p,
        dst_pts=x_edge_bubble,
        dst_teacher=x_prime,
        grid=grid_idx, locs=locs_ap, cmp=cmp0,
...
def tuning(_knob, _step):
    """Greedy 1D tweak: try stepping down first; if nothing improves, try stepping up."""
    layer = 5
    b_fpath = f"output/data/eb2_model_L{layer}_best.npz"
    teacher = load_eb_model_npz(b_fpath)
    if _knob not in teacher:
        print(f"knob {_knob!r} not found in model keys: {sorted(teacher.keys())}")

    status_quo = teacher.get(_knob)
    print(f'tuning {_knob}: {status_quo} ±{_step}')

    def _cast(v):
        if _knob.endswith("_m_end"):
            return int(round(float(v)))
        return float(v)

    tweak = _cast(status_quo)
    improved_any = False
    improved = True
    while improved:
        tweak = _cast(tweak - _step)
        teacher[_knob] = tweak
        improved = run(layer, b_fpath, teacher)
