...
def plot_density(b_oct, c_den, vts, trx, octant_id, layer, marker, oc_vtx=None):
    ...
    file_title = f"c3_{marker}_L{layer}"
    np.savez(
        f"output/{file_title}.npz",
        octant_id=octant_id,
        cmp=cmp,
        pts=vts,
        grid=trx
...
if __name__ == '__main__':
    ...
    h_start, h_stop, h_step = 0.23, 0.29, 7
    hyper = np.linspace(h_start, h_stop, h_step)
    print(f"g. hyper: ({h_start} to {h_stop}; {h_step} steps) across TAPER_FAC")
    for TAPER_FAC in hyper:
        ...
        # 2. Sort to guarantee topology on each edge ("zipper" fix)
        for j in range(len(lines)):
            ...
            # Current raw positions along this edge
            raw_t = best_t[indices]

            # Sort them so they don't cross (topology) while preserving distribution
            ...
            # Apply back to x_prime for the corresponding octant-edge vertex indices
