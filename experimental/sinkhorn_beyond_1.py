...
def prior_save_eb_model_npz(path: str,
                      *,
                      octant_id: int,
                      layer: int,
                      mode: int,
...
def prior_load_eb_model_npz(path: str) -> dict:
    repo = np.load(path, allow_pickle=True)
    out = {k: repo[k] for k in repo.files}
...
def canonicalize_xy(xy: np.ndarray, mode: int) -> tuple[np.ndarray, float]:
    """Map b_oct coords to canonical mode-1 via y-flip."""
...
def _edge_id_and_t_for_points(
    ...
    # Canonicalize so u/v/w meanings are stable across mode
...
def build_edge_maps(a_xy: np.ndarray,
    ...
    for j in range(3):
        ...
        maps[j]["t_map"] = _guard_edge_map_endpoints(maps[j]["t_raw"], maps[j]["t_map"], m_end=32, power=2.0)
    ...
    if enforce_lr_symmetry:
        ...
        maps[1]["t_map"] = _guard_edge_map_endpoints(maps[1]["t_raw"], maps[1]["t_map"], m_end=32, power=2.0)
        ...
        maps[2]["t_map"] = _guard_edge_map_endpoints(maps[2]["t_raw"], maps[2]["t_map"], m_end=32, power=2.0)
...
def run(a, u, r):
    ...
    verbose = True
    tri_detail = False
    cross_surface_detail = False

    eb_npz = f"output/data/cp_layer{layer}_eb.npz"
    best_path = f"output/data/eb_model_L{layer}_best.npz"

    # teacher_npz = "output/data/eb_model_L5.npz"
    # teacher_npz = "output/data/eb_model_L4.npz"   # <-- teacher is L4 model

    cmp0, a_p, v_ell, oc_vtx, oc_edg, grid_idx, g_locs = load_grid(layer)

    # corners for this mode (same for all layers)
    ...
    def run_edge_bubble(sens_w_in: np.ndarray, calc_alpha: bool = True):
        ...
        # rel_fine, _ = area_rel_err(rg, x_eb_loc, grid_idx, cmp0, quality=3)
        # print("final quality=3:", _alpha_obj_value(rel_fine))
    ...
    score = _model_score_from_rel(rel_fine, tri_mask)
    obj = _objective_from_score(score)
    print("candidate score:", score, "obj:", obj)
    latest_path = f"output/data/eb_model_L{layer}.npz"  # current path
    prev_obj = None
    ...
    tri_bad, vids_bad = print_worst_triangle(rg, x_edge_bubble, grid_idx, tri_mask, cmp0, locs_eb, "edge+bubble", detail=tri_detail)
    print('---------------- iteration done ----------------\n')
    return improved


if __name__ == "__main__":
    env_alpha_list = [0.45, 0.60, 0.80]
    eps_uvw_list = [0.0, 1e-5, 1e-4, -1.0]
    ridge_list = [0.0, 1e-6, 1e-5]

    for a in env_alpha_list:
        for u in eps_uvw_list:
            for r in ridge_list:
                for i in range(10):
                    print(a, u, r, i)
                    improved = run(a, u, r)   # make run() return True only when best improves
                    if not improved:
                        break