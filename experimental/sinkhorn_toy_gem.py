...
if __name__ == '__main__':
    ...
    # --- 2. Tuned Weights (Refined Profile) ---
    a_w = np.ones(num, dtype=np.float64) / num

    b_w_raw = np.exp(v_ell - np.median(v_ell))
    b_w_raw = np.clip(b_w_raw, np.percentile(b_w_raw, 2), np.percentile(b_w_raw, 98))

    # --- "FLATTENED ROOF" BOOST ---
    # Previous: Center=2.2 (Too Strong/Blue), Corner=1.0 (Too Weak/Red)
    # New:      Center=1.9 (Relaxed),        Corner=1.4 (Tightened)
    CENTER_BOOST = 1.62
    CORNER_BOOST = 1.55
    ...
    for idx in boundary_indices:
        ...
        # Taper over the first 15% of distance
        # 0.0 at corner, 1.0 at center of edge
        taper_factor = np.clip(d / 0.20, 0.0, 1.0)
    ...
    # Drop to 2e-4. The Log solver can handle this.
    # Lower reg = less inward "suction".
    print(f"Solving with Boost (reg={reg})...")
    ...
    for i, (v_start, v_end) in enumerate(lines):
        proj, t = project_and_get_t(edge_pts, v_start, v_end)
    ...
    plot_density(b_oct, rx, x_prime, t_grid, octant_id, layer, 'xp_high_boost')
